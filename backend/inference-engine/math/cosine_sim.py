...
# The core math utility that calculates the "Distance" between your taste and a potential show
...
def validate(user_vector, media_vector, weights):
    for name, vec in [("user_vector", user_vector), ("media_vector", media_vector), ("weights", weights)]:
...
def weighted_cosine(user_vector, media_vector, weights):
    numerator = sum(w * u * m for w, u, m in zip(weights, user_vector, media_vector))
    denom_user = math.sqrt(sum(w * u ** 2 for w, u in zip(weights, user_vector)))
    denom_media = math.sqrt(sum(w * m ** 2 for w, m in zip(weights, media_vector)))
...
def main():
    try:
        ...
        media_vector = payload["media_vector"]
    ...
    try:
        validate(user_vector, media_vector, weights)
    except ValueError as e:
        print(json.dumps({"error": str(e)}), file=sys.stderr)
        sys.exit(1)

    score = weighted_cosine(user_vector, media_vector, weights)
